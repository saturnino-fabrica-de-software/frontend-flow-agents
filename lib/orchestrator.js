  ...
  return setInterval(async () => {
    try {
      ...
      if (await fs.pathExists(tempDir)) {
        ...
        // Only update if there's actual change in agent progress
        if (currentStartFile && completeFiles.length !== lastAgentCount) {
          const agentName = currentStartFile.replace('_start.log', '').replace('agent_', '');
          const currentAgent = `agent_${agentName}`;
          const progress = Math.round((completeFiles.length / agentList.length) * 100);

          await updateStateFile(frontendFlowDir, {
            status: 'EXECUTING',
            currentAgent: currentAgent,
            currentTask: `Executando ${currentAgent}...`,
            progress: progress,
            agentsCompleted: completeFiles.length,
            event: 'agent_progress',
            recentActivity: `${currentAgent} em execução (${completeFiles.length}/${agentList.length})`
          });

          console.log(chalk.gray(`📊 Progresso: ${progress}% - ${currentAgent} (${completeFiles.length}/${agentList.length})`));
        }

        // Update completed agents count if it changed
        if (completeFiles.length > lastAgentCount) {
          ...
          await updateStateFile(frontendFlowDir, {
            status: 'EXECUTING',
            currentAgent: completeFiles.length < agentList.length ? 'pipeline_running' : 'pipeline_finalizing',
            currentTask: completeFiles.length < agentList.length ? 'Pipeline em execução...' : 'Finalizando pipeline...',
            progress: progress,
            agentsCompleted: completeFiles.length,
          ...
          console.log(chalk.green(`✅ Progresso: ${progress}% - ${completeFiles.length}/${agentList.length} agentes concluídos`));
        }
      }
    } catch (error) {
