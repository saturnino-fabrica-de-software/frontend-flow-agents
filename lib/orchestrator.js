...
async function runPipeline(demand, projectPath, options = {}) {
  ...
  // TEMPORARILY DISABLED: Enhanced mode is hanging
  const useEnhancedMode = false; // await checkEnhancedMode(frontendFlowDir, options);

  console.log('DEBUG: useEnhancedMode =', useEnhancedMode);
  if (useEnhancedMode) {
  ...
  console.log('DEBUG: After enhanced mode check');
  console.log(chalk.blue('🚀 Frontend Flow Standard - Iniciando...'));

  // Initialize Claude integration
  console.log('DEBUG: Before Claude init');
  const claude = new ClaudeIntegration();
  console.log('DEBUG: Before Claude detect');
  await claude.detectClaudeCode();
  console.log('DEBUG: After Claude detection');

  // Load configurations
  console.log('DEBUG: Before loadConfigurations');
  const configs = await loadConfigurations(frontendFlowDir);
  console.log('DEBUG: After loadConfigurations');
  ...
  console.log('DEBUG: Checking dryRun:', options.dryRun);
  if (options.dryRun) {
