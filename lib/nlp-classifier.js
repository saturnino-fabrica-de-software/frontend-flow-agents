...
class NLPClassifier {
  ...
  extractKeywords(content) {
    ...
    // Extract from frontmatter keywords (priority)
    const keywordsMatch = content.match(/keywords:\s*(.+)/);
    if (keywordsMatch) {
      const kws = keywordsMatch[1].split(',').map(k => k.trim());
  ...
  addAgentSpecificKeywords(content, keywords) {
    ...
    // Priority keywords for common tasks
    const priorityKeywords = {
      'component': ['agent_react_components', 'agent_vue_components'],
      'botão': ['agent_react_components'],
      'button': ['agent_react_components'],
      'style': ['agent_tailwind_estilization'],
      'css': ['agent_tailwind_estilization'],
      'test': ['agent_integration_tests', 'agent_playwright_validation'],
      'api': ['agent_api_integration', 'agent_nestjs_backend'],
      'backend': ['agent_nestjs_backend', 'agent_golang_backend'],
      'deploy': ['agent_deployment'],
      'git': ['agent_github_pullrequest'],
      'security': ['agent_security'],
      'performance': ['agent_performance']
    };

    // Check if demand contains priority keywords
    for (const [keyword, agents] of Object.entries(priorityKeywords)) {
      if (content.toLowerCase().includes(keyword)) {
        agents.forEach(agent => {
          if (agentName.includes(agent.replace('agent_', ''))) {
            // Boost priority for this agent
            keywords.add(keyword);
            keywords.add(keyword); // Double add for higher weight
          }
        });
      }
    }
  ...
  classifyDemand(demand, projectType = 'fullstack') {
    ...
    const priorityKeywords = {
      'botão': 'agent_react_components',
      'button': 'agent_react_components',
      'component': 'agent_react_components',
      'card': 'agent_react_components',
      'form': 'agent_react_components',
      'login': 'agent_react_components',
      'test': 'agent_integration_tests',
      'style': 'agent_tailwind_estilization',
      'css': 'agent_tailwind_estilization',
      'api': 'agent_api_integration',
      'backend': 'agent_nestjs_backend',
      'nest': 'agent_nestjs_backend',
      'pwa': 'agent_pwa_progressive',
      'offline': 'agent_pwa_progressive'
    };

    // Check priority keywords first
    for (const [keyword, agent] of Object.entries(priorityKeywords)) {
      if (demandLower.includes(keyword)) {
        matchedAgents.set(agent, 10); // High priority score
      }
    }

    // Score each agent based on keyword matches
    for (const [agentName, keywords] of this.agentPatterns) {
      let score = matchedAgents.get(agentName) || 0;
  ...
  classifyType(demand) {
    ...
    // Check PWA first (more specific)
    if (demandLower.includes('pwa') || demandLower.includes('offline') ||
        demandLower.includes('progressive web')) {
    ...
    if (demandLower.includes('component') || demandLower.includes('ui') ||
        ...
        demandLower.includes('card') || demandLower.includes('form') ||
        (demandLower.includes('criar') && (demandLower.includes('componente') || demandLower.includes('botão')))) {
