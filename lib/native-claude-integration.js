...
class NativeClaudeIntegration {
  constructor() {
    ...
    this.frontendFlowProjectDir = path.join(this.projectsDir, 'frontend-flow-agents');
  }

  async setupNativeIntegration(sourcePath, projectPath = null) {
    const spinner = createSpinner('Configurando integração nativa com Claude...').start();
    ...
    try {
      ...
      console.log(chalk.cyan('\n📦 Instalando Frontend Flow nativamente no Claude...'));

      // Copy entire agents structure to .claude/projects/frontend-flow-agents
      await this.copyAgentsToClaudeProjects(sourcePath);

      // Create or append to global CLAUDE.md with Frontend Flow instructions
      await this.updateClaudeMd(sourcePath);
      ...
      await configGenerator.createClaudeConfig(this.frontendFlowProjectDir);

      // If projectPath provided, also update project's CLAUDE.md
      if (projectPath) {
        await this.updateProjectClaudeMd(projectPath);
      }

      spinner.success(chalk.green('✅ Frontend Flow instalado nativamente!'));

      console.log(chalk.yellow('\n📝 Instruções:'));
      console.log(chalk.gray('1. Abra o Claude Code normalmente'));
      console.log(chalk.gray('2. Os agentes do Frontend Flow estarão disponíveis automaticamente'));
      console.log(chalk.gray('3. Use: "execute agent_react_components" ou qualquer outro agente'));
      console.log(chalk.gray('4. A mesa técnica estará integrada nativamente'));
  ...
  async copyAgentsToClaudeProjects(sourcePath) {
    console.log(chalk.blue('📂 Copiando agentes para ~/.claude/projects/frontend-flow-agents...'));

    // Ensure frontend-flow-agents directory exists in .claude/projects
    await fs.ensureDir(this.frontendFlowProjectDir);

    // Copy agents directory with all metadata
    const agentsSource = path.join(sourcePath, 'agents');
    const agentsDest = path.join(this.frontendFlowProjectDir, 'agents');
    ...
    const templatesDest = path.join(this.frontendFlowProjectDir, 'templates');
    ...
    const configsDest = path.join(this.frontendFlowProjectDir, 'configs');
...
`;
    ...
    await fs.writeFile(
      path.join(this.frontendFlowProjectDir, 'PROJECT.md'),
      projectMd,
      'utf8'
    );
  ...
  async updateClaudeMd(sourcePath) {
    console.log(chalk.blue('📝 Atualizando CLAUDE.md global...'));
...
`;
  ...
  async updateProjectClaudeMd(projectPath) {
    console.log(chalk.blue('📝 Atualizando CLAUDE.md do projeto...'));

    const projectClaudeMdPath = path.join(projectPath, 'CLAUDE.md');
...
`;
  ...
  async copyTemplatesAndConfigs(sourcePath) {
    ...
    const libDest = path.join(this.frontendFlowProjectDir, 'lib');
  ...
  async checkNativeIntegration() {
    try {
      const exists = await fs.pathExists(this.frontendFlowProjectDir);
      if (exists) {
        const agentsDir = path.join(this.frontendFlowProjectDir, 'agents');
  ...
  async removeNativeIntegration() {
    const spinner = createSpinner('Removendo integração nativa...').start();

    try {
      if (await fs.pathExists(this.frontendFlowProjectDir)) {
        await fs.remove(this.frontendFlowProjectDir);
        spinner.success(chalk.green('✅ Integração nativa removida'));

        console.log(chalk.yellow('\n⚠ Nota: A seção Frontend Flow em CLAUDE.md foi mantida.'));
        console.log(chalk.gray('  Para remover completamente, edite ~/.claude/CLAUDE.md manualmente.'));
      } else {
        spinner.info(chalk.yellow('ℹ Integração nativa não encontrada'));
      }
    } catch (error) {
