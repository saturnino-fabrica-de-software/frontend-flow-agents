  ...
  async callClaudeCodeInteractive(agentContext, projectPath, options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (options && options.promptFile) {
        ...
        prompt = this.buildAgentPrompt(agentContext);
      }
      ...
      // If promptFile is provided, read the file and pass as prompt
      const claudeArgs = options && options.promptFile
        ? ['-c', `cat "${prompt}" | ${this.claudePath}`]
        : [prompt];

      const claudeCommand = options && options.promptFile ? '/bin/bash' : this.claudePath;
