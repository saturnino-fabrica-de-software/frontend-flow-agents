  ...
  async callClaudeCode(agentContext, projectPath, options) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Use Claude Code with --print flag for non-interactive execution
        // and --permission-mode to handle permissions automatically
        const command = `${this.claudePath} --print --permission-mode acceptEdits "${prompt}"`;

        if (options.verbose) {
          console.log(chalk.gray(`Executing: ${command.substring(0, 100)}...`));
        }

        exec(command, {
          cwd: projectPath,
          timeout: options.timeout || 180000, // 3 minutes for real work
